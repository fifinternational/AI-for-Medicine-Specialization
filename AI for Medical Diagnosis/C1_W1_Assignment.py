...
def check_for_leakage(df1, df2, patient_col):
    ...
    Args:
        df1 (dataframe): DataFrame describing the first dataset
        df2 (dataframe): DataFrame describing the second dataset
        patient_col (str): String name of the column with patient IDs
    
    Returns:
        leakage (bool): True if there is leakage, otherwise False
    """

    # Extract unique patient IDs from both dataframes
    df1_patients_unique = set(df1[patient_col].unique())
    df2_patients_unique = set(df2[patient_col].unique())
    
    # Check for any overlap between the two sets of patient IDs
    patients_in_both_groups = df1_patients_unique.intersection(df2_patients_unique)
    
    # Leakage contains True if there is at least 1 patient in both groups, otherwise False
...
positive_frequencies, negative_frequencies = compute_class_freqs(labels)

print("Positive Frequencies:", positive_frequencies)
print("Negative Frequencies:", negative_frequencies)
...
# Example data
labels = np.array(["Class1", "Class2", "Class3"])  # Replace with actual class labels
freq_pos = np.array([0.8, 0.4, 0.8])  # Replace with actual positive frequencies
freq_neg = np.array([0.2, 0.6, 0.2])  # Replace with actual negative frequencies
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        for i in range(len(pos_weights)):
            # Extract true and predicted values for the i-th class
            y_true_class = y_true[:, i]
            y_pred_class = y_pred[:, i]
            
            # Calculate positive and negative parts of the loss using Keras operations
            pos_loss = -pos_weights[i] * K.mean(y_true_class * K.log(y_pred_class + epsilon))
            neg_loss = -neg_weights[i] * K.mean((1 - y_true_class) * K.log(1 - y_pred_class + epsilon))

            # Add the weighted loss for this class to the total loss
